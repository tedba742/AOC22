...
public:
    ...
    Node* findNodePointingTo(std::string filename) {
        ...
        return nullptr; // If no node pointing to the current node is found
    }
...
void printNodesWithSmallNeighbors(long threshold) {
  long long totalSize= 0;
     for (Node* node : nodes) {
        long neighborSum = 0;
        for (Node* neighbor : node->neighbors) {
            neighborSum += neighbor->fileSize;
        }
        if (neighborSum < threshold) {
    totalSize += neighborSum;
              }
    }
...
int main() {
    ...
    // Printing the graph (before adding edges, so it will show only nodes)
    graph.print();

    // Calculate and print the sum of file sizes of all neighbors of a node
