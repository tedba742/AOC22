...
def find_shortest_path(grid, start, goal, initial_time, blizzards_positions):
    rows = len(grid)
    cols = len(grid[0])
    ...
    queue = deque([(initial_time, start)])
    visited = set([(initial_time, start)])
...
if __name__ == "__main__":
    ...
    time_to_goal = find_shortest_path(grid, start, goal, 0, blizzards_positions)
    
    # Second trip: Goal back to Start
    time_back_to_start = find_shortest_path(grid, goal, start, time_to_goal, blizzards_positions)
    
    # Third trip: Start to Goal again
    time_back_to_goal = find_shortest_path(grid, start, goal, time_back_to_start, blizzards_positions)
    
    total_time = time_back_to_goal
    print("Fewest number of minutes required to reach the goal, go back to the start, then reach the goal again:", total_time)
